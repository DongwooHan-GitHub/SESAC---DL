...
def word2tensor(word, max_length, alphabets, pad = PAD, oov = OOV):
    #print(f"alphabets_length :{len(alphabets)}")
...
def generate_dataset2(batch_size = 32):
    ...
    # print(f"len(alphabets) : {len(alphabets)}")
    ...
    def train_model(self, train_data, valid_data, epochs=100, learning_rate=0.001, print_every=1000):
        ...
        for epoch in range(epochs):
            for x, y in train_data:
                ...
                if self.batch_first:
                    x = x.transpose(0, 1)  # (batch_size, max_length, input_size) -> (max_length, batch_size, input_size)

                # 초기 은닉 상태를 (batch_size, hidden_size) 형태로 설정
                hidden = self.init_hidden(x.size(1))  # x.size(1)로 배치 크기 사용
                ...
                for char in x:
                    # print(f"char : {char.shape}")
                    # char = char.view(1, -1)  # char를 (1, input_size) 형태로 변환
                ...
                mean_loss = loss.item()  # 손실 값 계산

                if step % print_every == 0 or step == 1:
                    train_loss_log.append(mean_loss)
                    ...
                    # print(f'[Epoch {epoch}, Step {step}] train loss: {mean_loss}')
...
dataloader, alphabets, max_length, languages = generate_dataset2()
train_dataloader, valid_dataloader, test_dataloader = modify_dataset_for_ffn(dataloader)  
# print(len(alphabets))
hidden_size = 32
rnn = RNN(hidden_size, alphabets, languages)
train_loss_history, valid_loss_history = rnn.train_model(train_dataloader, valid_dataloader)
